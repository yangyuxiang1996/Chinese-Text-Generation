...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        src_encodings, (h, c) = self.lstm(embedded.permute(1, 0, 2))
        return src_encodings.permute(1, 0, 2), (h, c)
...
class Decoder(nn.Module):
    ...
    def forward(self, x_t, decoder_states, context_vector):
        ...
        decoder_output, decoder_states = self.lstm(decoder_emb.permute(1, 0, 2),
                                                   decoder_states)
    ...
    def forward(self, hidden, img_vec=None):
        ...
        Args:
            hidden (tuple):
                ...
                each with shape (2, batch_size, hidden_units).
            img_vec (Tensor):
                The image vectors with shape (batch_size, image_vector_dim)
        ...
        # if config.img_feat:
        #     h_reduced = torch.tanh(self.init_decoder_cell1(torch.cat([h_reduced, img_vec.unsqueeze(0)], dim=-1)))
        #     c_reduced = torch.tanh(self.init_decoder_cell1(torch.cat([c_reduced, img_vec.unsqueeze(0)], dim=-1)))
...
class PGN(nn.Module):
    ...
    def load_model(self):
        ...
        if (os.path.exists(config.encoder_save_name)):
            ...
            self.reduce_state = torch.load(f'../saved_model/{config.model_for_ft}/reduce_state.pt')
    ...
    def get_final_distribution(self, x, p_gen, p_vocab, attention_weights,
        ...
        final_distribution = \
            p_vocab_extended.scatter_add_(dim=1,
                                          index=x,
                                          src=attention_weighted)  + config.eps

        return final_distribution

#     @timer('model forward')
    def forward(self, x, y, len_oovs, img_vec, batch, num_batches):
