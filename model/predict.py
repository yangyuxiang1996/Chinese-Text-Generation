...
#         log_probs = torch.distributions.Categorical(log_probs)
        topk_probs, topk_idx = torch.topk(log_probs, k, dim=0)
    ...
    def predict(self, x, oov=None, img_vec=None, tokenize=True, beam_search=True):
        """Generate summary.
        ...
        """
        if isinstance(x, str) and tokenize:
            x = list(jieba.cut(x))
            x, oov = source2ids(x, self.vocab)
...
if __name__ == "__main__":
    ...
    with open(config.test_data_path, 'r') as testset:
        picked = random.choice(list(testset))
        print('source: ', picked, '\n')
